...
city_urls = {'bj': 'https://bj.lianjia.com/ershoufang/pg',
             'sh': 'http://sh.lianjia.com/ershoufang/d',
             'hz': 'https://hz.lianjia.com/ershoufang/pg',
             'sz': 'https://sz.lianjia.com/ershoufang/pg',
             'gz': 'https://gz.lianjia.com/ershoufang/pg'}
...
for city in city_urls.keys():
    ...
    file_name = cur_file_dir() + os.path.sep + city + '_data_' + time.strftime('%Y%m%d%H%M%S') + '.txt'
    ...
    while True:
        ...
        if city == 'sh':
            ...
            ele_list = soup.select('body > div.content > div.leftContent > ul > li')

            if len(ele_list) <= 0:
                break

            for ele in ele_list:
                title = ele.select_one('div.info.clear > div.title > a').text
                title = title.replace('\t', '').replace('\n', '').replace(' ', '').replace('|', '')

                address = ele.select_one('div.info.clear > div.address > div').text
                address = address.replace('\t', '').replace('\n', '').replace(' ', '')
                addresses = address.split('|')
                split_string = '|||||'

                if len(addresses) < 6:
                    address = address + split_string[0:6 - len(addresses)]

                flood = ele.select_one('div.info.clear > div.flood > div').text
                flood = flood.replace('\t', '').replace('\n', '').replace(' ', '').replace('-', '|')

                total_price = ele.select_one('div.info.clear > div.priceInfo > div.totalPrice').text
                total_price = total_price.replace('\t', '').replace('\n', '').replace(' ', '')

                unit_price = ele.select_one('div.info.clear > div.priceInfo > div.unitPrice').text
                unit_price = unit_price.replace('\t', '').replace('\n', '').replace(' ', '')

                file.write(title + '|' + address + '|' + flood + '|' + total_price + '|' + unit_price + '\n')
